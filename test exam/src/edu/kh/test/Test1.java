...
public class Test1 {
	public static void main(String[] args) {
		...
		// ipInfoList는 ArrayList로 생성해야 하지만, 
		// null로 대입하였으므로 ipInfoList에 요소를 추가(add)하는 13~15 문장은 NullPointerException이 발생 
		
		ipInfoList.add(new IPInfo("123.123.123.123","홍길동"));
		ipInfoList.add(new IPInfo("212.133.7.8","김기태")); 
		// 방법1) IPInfo 클래스에 매개 변수 ip 만 받는 생성자를 추가 
		// 방법2) IPInfo 객체 생성시 ip와 user의 매개변수 값을 모두 넣는다. 
		// ipInfoList.add(new IPInfo("212.133.7.8"));  
		// IPInfo 클래스에서 매개변수 하나만 전달 받는 생성자(Constructor)는 없으므로 객체 생성 불가  
		...
		for(int i=0 ; i<ipInfoList.size() ; i++) {
			if(ipInfoList.get(i).getIp().equals(ip)) { 
				...
				// ipInfoList.get(i)는 특정 번째의 IPInfo 클래스의 객체이고, ip 는 String 변수 이므로, 
